    ...
    def render_tex
      ...
      infiles.each do |infilepath|
        ...
        if infilepath.include? '.page'
          kramdown = Publishr::Helper.strip_webgen_header_from_page_file(content)
        else
          kramdown = content
        end
        latex = Kramdown::Document.new(kramdown, @metadata['kramdown_options']).to_latex
        fixed_latex = LatexProcessor.new(@inpath, @outpath, File.basename(infilepath), @language).fix(latex)
